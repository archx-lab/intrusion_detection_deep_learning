...
actual = test_dataset.iloc[:, -2].values

print(np.asarray(np.unique(y_train, return_counts=True)))
print(np.asarray(np.unique(actual, return_counts=True)))
...
cnn.load_weights("results/cnn_model.hdf5")

y_pred = cnn.predict_classes(X_test)

'''np.savetxt('res/expected3.txt', actual1, fmt='%01d')
...
loss, accuracy = cnn.evaluate(X_test, actual)
...
cm = multilabel_confusion_matrix(actual, y_pred)
...
print('Precision Score : ', np.mean(precision_score(actual, y_pred, average = None)))
print('Recall Score : ', np.mean(recall_score(actual, y_pred, average = None)))
print('F1 Score : ', np.mean(f1_score(actual, y_pred, average = None)))
