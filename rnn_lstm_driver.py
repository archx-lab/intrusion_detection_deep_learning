...
acc_mentioned=76.11
def driver_func(var,train_dataset,test_dataset,train_dataset_original,syn_data_list,acc_mentioned):
    global accept
    if(len(accept)!=0):
        for j in accept:
    ...
    batch_size = 64
    model = Sequential()
    model.add(LSTM(64,input_dim=116, return_sequences=True))  # try using a GRU instead, for fun
    ...
    model.compile(loss="sparse_categorical_crossentropy", optimizer="adam",metrics=[tf.keras.metrics.CategoricalAccuracy()])
...
for var in range(5):
    driver_func(var,train_dataset,test_dataset,train_dataset_original,syn_data_list,acc_mentioned)
